Encoder = {
    ...
    isEmpty: function(val) {
        return val ? null === val || 0 == val.length || /^\s+$/.test(val) : true;
    },
    ...
    hasEncoded: function(s) {
        return /&#[0-9]{1,5};/g.test(s) ? true : /&[A-Z]{2,6};/gi.test(s) ? true : false;
    },
    ...
    swapArrayVals: function(s, arr1, arr2) {
        if (this.isEmpty(s)) return "";
        var re;
        if (arr1 && arr2 && arr1.length == arr2.length) for (var x = 0, i = arr1.length; i > x; x++) {
